...
var staffwood_meteorite = VanillaFactory.createItem("staffwood_meteorite");
    staffwood_meteorite.maxStackSize = 1;
    staffwood_meteorite.maxDamage = 20;
    staffwood_meteorite.toolLevel = 1;
    staffwood_meteorite.onItemUse = function(player, world, pos, hand, facing, blockHit){
        ...
        if (hand == mods.contenttweaker.Hand.main()){
            ...
            if (!world.isRemote()){
                ...
                var stack as MutableItemStack = player.getHeldItem(mods.contenttweaker.Hand.main());
                var ent_pos = pos.getOffset(facing, 1);
                var ent_pos_str as string = "" ~ ent_pos.x ~ " ~200 " ~ ent_pos.z;

                Commands.call("summon minecraft:falling_block " ~ ent_pos_str ~ " {Block:\"taiga:meteorite_block\",Time:1,Motion:[0.0d,-1.0d,0.0d]}", player, world, false, true);
                stack.damage(1, player);
...
var staffwood_fey = VanillaFactory.createItem("staffwood_fey");
    staffwood_fey.maxStackSize = 1;
    staffwood_fey.maxDamage = 250;
    /*staffwood_fey.onItemUse = function(player, world, pos, hand, facing, blockHit) {
        var block = world.getBlockState(pos);
        if (block == <block:quark:biome_cobblestone:1>) {
            world.setBlockState(<block:aether_legacy:icestone>, pos);
            player.getHeldItem(hand).damage(1, player);
